# Modified renderer from torch-ngp 
...
class NeRFRenderer(MetaModule):
    ...
    def run(self, rays_o, rays_d,latent_code=None, num_steps=128, upsample_steps=128, bg_color=None, perturb=False,params=None, idx=None, **kwargs):
        ...
        rays_o_nograd = rays_o.detach().clone()
        rays_d_nograd = rays_d.detach().clone()
        ...
        # sample steps
        nears, fars = raymarching.near_far_from_aabb(rays_o_nograd, rays_d_nograd, aabb, self.min_near)
        nears.unsqueeze_(-1)
        fars.unsqueeze_(-1)
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3),params=params, idx=idx)

        # print(f"Density Outputs: {density_outputs['sigma'].shape}")

        #sigmas = density_outputs['sigma'].view(N, num_steps) # [N, T]
        for k, v in density_outputs.items():
            if density_outputs[k].isinf().any(): 
                torch.clamp(density_outputs[k], max=1e6)
            if density_outputs[k].isnan().any():
                import pdb 
                pdb.set_trace()
        ...
        rgbs = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1),params=params, **density_outputs)
        ...
        weights_sum = weights_sum.view(*prefix)
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096,params=None,idx=None, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail],params=params, idx=idx, **kwargs)
            ...
            results = _run(rays_o, rays_d,params=params,idx=idx, **kwargs)
